...
function simpl(options) {
  return function (Component) {
    ...
    console.log("optionsWithDefaults.loadAllScenarios: ".concat(optionsWithDefaults.loadAllScenarios));
    console.log("optionsWithDefaults.loadWorldResults: ".concat(optionsWithDefaults.loadWorldResults));
    console.log("optionsWithDefaults.loadRunDataOnDemand: ".concat(optionsWithDefaults.loadRunDataOnDemand));
        ...
        onReady: function onReady() {
          ...
          if (optionsWithDefaults.topics) {
            optionsWithDefaults.topics.forEach(function (topic) {
              console.log("dispatching connectedScope(".concat(topic, ")"));
              dispatch((0, _simpl.connectedScope)(topic));
              console.log("dispatching getRunUsers(".concat(topic, ")"));
              dispatch((0, _simpl.getRunUsers)(topic)).then(function (action) {
                ...
                for (var i = 0; i < runUsers.length; i++) {
                  ...
                  if (optionsWithDefaults.loadAllScenarios) {
                    ...
                    console.log("dispatching getRunUserScenarios(".concat(ruTopic, ")"));
                ...
                console.log("dispatching getCurrentRunUserInfo(".concat(authid, ")"));
                dispatch((0, _simpl.getCurrentRunUserInfo)(authid));
              });
              console.log("dispatching getCurrentRunPhase(".concat(topic, ")"));
              dispatch((0, _simpl.getCurrentRunPhase)(topic));

              if (options.loadRunDataOnDemand) {
                console.log("Will load data tree and subscribe to topic ".concat(topic, " on demand."));
                console.log("dispatching getDataTree(".concat(topic, ", ['world'])"));
                dispatch((0, _simpl.getDataTree)(topic, ['world']));
              } else {
                if (!optionsWithDefaults.loadWorldResults && topic.includes('run')) {
                  console.log("dispatching getDataTree(".concat(topic, ", ['result'])"));
                  dispatch((0, _simpl.getDataTree)(topic, ['result']));
                } else {
                  console.log("dispatching getDataTree(".concat(topic, ")"));
                  dispatch((0, _simpl.getDataTree)(topic));
                }

                console.log("dispatching addTopic(".concat(topic, ")"));
