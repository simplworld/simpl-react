  ...
  return function (Component) {
    ...
    var mapDispatchToProps = function mapDispatchToProps(dispatch) {
      return {
            ...
            optionsWithDefaults.topics.forEach(function (topic) {
              ...
              dispatch((0, _simpl.getRunUsers)(topic)).then(function (action) {
                ...
                for (var i = 0; i < runUsers.length; i++) {
                  ...
                  if (optionsWithDefaults.loadAllScenarios) {
                  ...
                  } else if (ru.data.email === authid) {
                    // only get this user's scenarios
                    // console.log(`dispatching getRunUserScenarios(${ruTopic})`);
                    dispatch((0, _simpl.getRunUserScenarios)(ruTopic)); // Subscribe to this user's chat rooms.  We subscribe to these
                    ...
                    console.log("dispatching ".concat(options.root_topic, ".chat.rooms_for_user ").concat(ru.data.id));
                    dispatch((0, _simpl.getRunUserChatRooms)(options.root_topic, ru.data.id)).then(function (action) {
                      ...
                      rooms.forEach(function (room) {
                        console.log("Loading messages from room ".concat(room.slug));
                        dispatch((0, _simpl.loadChatMessages)(options.root_topic, room.slug, authid));
                      });
                    });
                    break;
                  }
        ...
        onReceivedWithTopics: function onReceivedWithTopics(args, kwargs, event, topics) {
          ...
          if (event.topic.startsWith("".concat(options.root_topic, ".chat.room"))) {
            // console.log(`Received chat message:`, args);
            // console.dir(args)
            dispatch((0, _simpl.receiveChatMessage)(args[0]));
            return;
          }
    ...
    function (_React$Component2) {
      ...
      _createClass(Simpl, [{
        ...
        key: "render",
        value: function render() {
          var addonTopics = ["model:error.".concat(options.authid)];
          this.props.rooms.forEach(function (room) {
            addonTopics.push("".concat(options.root_topic, ".chat.room.").concat(room.slug));
          });
