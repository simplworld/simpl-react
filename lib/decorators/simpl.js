...
import { addChild, getDataTree, updateScope } from '../actions/simpl'
  ...
  const appMapDispatchToProps = (dispatch, ownProps) => {
    return {
      onReceived([pk, resource_name, data], kwargs, event) {
        ...
        if (event.topic == `${topic}.add_child`) {
          dispatch(addChild({resource_name: resource_name, data: data, pk: pk}))
        }
        if (event.topic == `${topic}.update_child`) {
          console.log(resource_name)
          dispatch(updateScope({resource_name: resource_name, data: data, pk: pk}))
        }
      }
    }
  }
