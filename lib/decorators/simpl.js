...
function simpl(options) {
  return function (Component) {
    ...
    var mergeProps = function mergeProps(propsFromState, propsFromDispatch) {
      return _extends({}, propsFromState, propsFromDispatch, {
            ...
            optionsWithDefaults.topics.forEach(function (topic) {
              ...
              dispatch((0, _simpl.getRunUsers)(topic)).then(function (action) {
                ...
                for (var i = 0; i < runUsers.length; i++) {
                  var ru = runUsers[i];
                  var runuserTopic = 'model:model.runuser.' + ru.data.id;
                  if (optionsWithDefaults.loadAllScenarios) {
                    // console.log(`dispatching getRunUserScenarios(${runuserTopic})`);
                    dispatch((0, _simpl.getRunUserScenarios)(runuserTopic));
                    dispatch((0, _simpl.addTopic)(runuserTopic)); // subscribe to all runuser topics
