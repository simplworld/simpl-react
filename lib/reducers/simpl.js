...
const simpl = recycleState(createReducer(initial, {
  ...
  [SimplActions.getRunUsers](state, action) {
    if (action.payload.error) {
      return this._handleError(state, action)
    }
    return action.payload.reduce((memo, child) => {
      return this._addChild(memo, {payload: child})
    }, Object.assign({}, state))
  },
  [SimplActions.getDataTree](state, action) {
    ...
    return Object.assign({}, this._getDataTree(Object.assign({}, state), action), {loaded: true})
  },
