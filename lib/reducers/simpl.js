...
var simpl = (0, _reduxRecycle.default)((0, _reduxCreateReducer.createReducer)(initial, (_createReducer = {
  ...
  silentRemoveTopic: function silentRemoveTopic(state, payload) {
    // console.log('silentRemoveTopic: payload:', payload);
    var topic = "model:model.".concat(payload.resource_name, ".").concat(payload.pk);

    var newState = _objectSpread({}, state);

    var index = newState.topics.indexOf(topic);

    if (index !== -1) {
      newState.topics = (0, _collections.popAtIndex)(newState.topics, index);
    }

    return _objectSpread({}, newState);
  },
  silentRemoveChild: function silentRemoveChild(state, payload) {
    ...
    if (index !== -1) {
      newState[resourceName] = (0, _collections.popAtIndex)(newState[resourceName], index);
    }
  ...
  var worlds = newState.world;

  if (!_lodash.default.isEmpty(worlds)) {
    worlds.forEach(function (world) {
      // console.log('remove topic for world:', world);
      newState = _this4.silentRemoveTopic(newState, world); // console.log('bulk unload children of world');
      ...
      scenarios.forEach(function (scenario) {
        ...
        periods.forEach(function (period) {
          ...
          decisions.forEach(function (decision) {
            newState = _this4.silentRemoveChild(newState, decision);
          });
          var results = state.result.filter(function (r) {
            return period.id === r.period;
          });
          results.forEach(function (result) {
            newState = _this4.silentRemoveChild(newState, result);
          });
          newState = _this4.silentRemoveChild(newState, period);
        });
        newState = _this4.silentRemoveChild(newState, scenario);
      });
      newState = _this4.silentRemoveChild(newState, world);
    });
  }
