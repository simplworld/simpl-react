...
var simpl = (0, _reduxRecycle2.default)((0, _reduxCreateReducer.createReducer)(initial, (_createReducer = {
  ...
  addTopics: function addTopics(state, newTopics) {
    var topics = [].concat(_toConsumableArray(state.topics), _toConsumableArray(newTopics));
    return _extends({}, state, { topics: topics });
  },
  removeTopic: function removeTopic(state, removedTopic) {
    var topic = 'model:model.world.' + removedTopic.pk;
    console.log('removeTopic: topic: ', topic);
    var index = state.topics.findIndex(topic);
  ...
  var index = newState[resourceName].findIndex(function (scope) {
    return scope.pk === newState.payload.pk;
  });
  if (index === -1) {
