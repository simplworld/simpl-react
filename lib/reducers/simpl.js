...
}), _defineProperty(_createReducer, SimplActions.getRunUserChatRooms, function (state, action) {
  // Get the current user's info into the current_runuser namespace
  var rooms = action.payload;
  return _extends({}, state, {
    chatrooms: rooms
  });
}), _defineProperty(_createReducer, SimplActions.loadChatMessages, function (state, action) {
  var newMessages = action.payload;
  var messages = state.messages.concat(newMessages);
  return _extends({}, state, {
    messages: messages
  });
}), _defineProperty(_createReducer, SimplActions.receiveChatMessage, function (state, action) {
  var newMessage = action.payload;

  var messages = _toConsumableArray(state.messages).concat([newMessage]);
