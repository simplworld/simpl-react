...
var simpl = (0, _reduxRecycle.default)((0, _reduxCreateReducer.createReducer)(initial, (_createReducer = {
  ...
  removeTopic: function removeTopic(state, payload) {
    var topic = "model:model.".concat(payload.resource_name, ".").concat(payload.pk);
    ...
    var updated = (0, _collections.popAtIndex)(state.topics, index);
  ...
  if (action.payload.resource_name === 'world') {
    var topic = "model:model.world.".concat(action.payload.pk);
  ...
  console.log('SimplActions.getRunUserScenarios: connectionStatus=', connectionStatus, ', treeLoaded=', state.treeLoaded, ', scenariosLoaded=', state.scenariosLoaded, ', rolesLoaded', state.rolesLoaded, ', phasesLoaded', state.phasesLoaded);
  ...
  console.log('SimplActions.getPhases: connectionStatus=', connectionStatus, ', treeLoaded=', state.treeLoaded, ', scenariosLoaded=', state.scenariosLoaded, ', rolesLoaded', state.rolesLoaded, ', phasesLoaded', state.phasesLoaded);
  return _extends({}, state, {
  ...
  console.log('SimplActions.getRoles: connectionStatus=', connectionStatus, ', treeLoaded=', state.treeLoaded, ', scenariosLoaded=', state.scenariosLoaded, ', rolesLoaded', state.rolesLoaded, ', phasesLoaded', state.phasesLoaded);
  return _extends({}, state, {
    role: action.payload,
    rolesLoaded: true,
    connectionStatus: connectionStatus
  ...
  console.log('SimplActions.setConnectionStatus: connectionStatus=', connectionStatus, ', treeLoaded=', state.treeLoaded, ', scenariosLoaded=', state.scenariosLoaded, ', rolesLoaded', state.rolesLoaded, ', phasesLoaded', state.phasesLoaded);
  return _extends({}, state, {
...
}), _defineProperty(_createReducer, SimplActions.setCurrentRun, function (state, action) {
  console.log('SimplActions.setCurrentRun: action: ', action);
  var currentRunId = action.payload.pk;
  console.log('currentRunId:', currentRunId);
  var currentRun;
  state.run.forEach(function (run) {
    if (run.pk === currentRunId) {
      currentRun = run; // protect from page refreshes
...
}), _defineProperty(_createReducer, SimplActions.getCurrentRun, function (state, action) {
  console.log('SimplActions.getCurrentRun: action: ', action);
  var currentRun = state.current_run;

  if ((0, _lodash.isEmpty)(currentRun)) {
    // check for cached current run id
    var currentRunId = sessionStorage.getItem('current_run_id');
    console.log('sessionStorage current_run_id:', currentRunId);

    if (!(0, _lodash.isNil)(currentRunId)) {
      state.run.forEach(function (run) {
        if (run.pk == currentRunId) {
          currentRun = run;
        }
