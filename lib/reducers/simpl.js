...
}), _defineProperty(_createReducer, SimplActions.setLoadedRun, function (state, action) {
  console.log('SimplActions.setLoadedRun: action: ', action);
  var loadedRun = action.payload;
  return _extends({}, state, {
    loaded_run: loadedRun
  });
}), _defineProperty(_createReducer, SimplActions.unloadWorlds, function (state, action) {
  console.log('SimplActions.unloadWorlds: action: ', action);
  var loadedRun = action.payload;

  if (_lodash.default.isEmpty(loadedRun)) {
    return state;
  }

  var newState = _objectSpread({}, state);

  var worlds = state.world.filter(function (w) {
    return loadedRun.id === w.run;
  });
