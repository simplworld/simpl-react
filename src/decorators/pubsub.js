...
import { Connection } from 'autobahn-react'
import { connect } from 'react-redux'


const AutobahnPromise = new Promise((resolve, reject) => {
  Connection.onReady(([session, details]) => {
    resolve(session)
  });
})
...
export function publishes(Component, topic, options = {}) {
  ...
  const Publisher = React.createClass({
    publish(payload) {
      ...
      return Autobahn.publish(topic, args, kwargs, options)
      .then(publication => {
